...
<?php
    ...
    <form method="POST" action="actions/add_password.php" class="bg-[#1a1a1a] border border-gray-800 p-6 rounded-2xl mb-12 shadow-xl">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            <div><label class="text-xs text-gray-500 ml-1 uppercase tracking-wider font-bold">Email</label><input type="email" name="email" required placeholder="mail@example.com" class="w-full bg-[#0f0f0f] border border-gray-800 px-4 py-2 rounded-xl focus:ring-1 focus:ring-blue-500 outline-none mt-1"></div>
...
<div class="mt-8 pt-6 border-t border-gray-800/50">
    <button type="button" onclick="openChangeMasterModal()"
        class="w-full py-4 bg-gray-900/50 border border-gray-700 text-gray-300 rounded-xl hover:border-blue-500 hover:text-blue-400 transition-all font-bold uppercase text-[10px] tracking-widest flex items-center justify-center gap-2">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"/></svg>
        Change Master Password
    </button>
</div>

<div class="mt-10 pt-6 border-t border-gray-800/50">
    <h3 class="text-red-500 font-bold mb-4 uppercase text-[10px] tracking-widest flex items-center gap-2">
        <span class="w-2 h-2 bg-red-500 rounded-full animate-pulse"></span>
        Danger Zone
    </h3>
    
    <button type="button" onclick="openTerminateModal()"
        class="relative w-full py-4 bg-gradient-to-r from-red-600/20 to-red-700/20 hover:from-red-600 hover:to-red-700 text-red-500 hover:text-white border border-red-900/30 rounded-xl shadow-lg transition-all duration-300 font-bold uppercase text-xs tracking-widest flex items-center justify-center gap-3 active:scale-[0.98] group">
        <svg class="w-5 h-5 transition-transform group-hover:rotate-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
        </svg>
        <span>Terminate Account & Vault</span>
    </button>
</div>

<div id="terminate-modal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-md z-[100] items-center justify-center p-4 transition-all duration-300">
    <div class="bg-[#1a1a1a] border border-gray-800 w-full max-w-md rounded-2xl shadow-2xl p-8 transform scale-95 opacity-0 transition-all duration-300" id="terminate-content">
        
        <div class="text-center mb-6">
            <div class="bg-red-500/10 w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 border border-red-500/20 rotate-3 group-hover:rotate-0 transition-transform">
                <svg class="w-8 h-8 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                </svg>
            </div>
            <h2 class="text-xl font-bold text-white tracking-tight">Final Warning</h2>
            <p class="text-gray-400 text-xs mt-3 leading-relaxed">
                This action is <span class="text-red-400 underline decoration-red-900">irreversible</span>. Everything in your vault will be permanently deleted.
            </p>
            <div class="mt-4 p-3 bg-red-900/10 border border-red-900/20 rounded-lg">
                <p class="text-[10px] text-gray-400 uppercase font-bold tracking-tighter mb-1">Type your username to confirm:</p>
                <span class="text-emerald-400 font-mono text-sm select-none"><?= htmlspecialchars($_SESSION['username']) ?></span>
            </div>
        </div>

        <form action="actions/delete_account.php" method="POST" id="terminate-form">
            <div class="mb-6">
		<input type="hidden" name="csrf_token" value="<?= $csrf_token ?>">
                <input type="text" id="username-confirm-input" autocomplete="off"
                    placeholder="Enter username" 
                    class="w-full bg-[#0f0f0f] border border-gray-800 px-4 py-3 rounded-xl focus:ring-1 focus:ring-red-500 outline-none text-white text-center transition-all placeholder:text-gray-700">
            </div>

            <div class="flex gap-3">
                <button type="button" onclick="closeTerminateModal()" 
                    class="flex-1 py-3 bg-gray-800 hover:bg-gray-700 text-gray-300 rounded-xl font-bold uppercase text-[10px] tracking-widest transition-all">
                    Cancel
                </button>
                <button type="submit" id="final-terminate-btn" disabled 
                    class="flex-1 py-3 bg-red-600/10 text-red-500/30 rounded-xl font-bold uppercase text-[10px] tracking-widest cursor-not-allowed transition-all shadow-none">
                    Delete All
                </button>
            </div>
        </form>
...
<div id="change-master-modal" class="fixed inset-0 bg-black/90 hidden items-center justify-center z-[120] p-4 backdrop-blur-md">
    <div class="bg-[#1a1a1a] border border-gray-800 rounded-2xl p-8 max-w-sm w-full">
        <h3 class="text-white font-bold mb-5">Change Master Password</h3>
        <form action="actions/change_master.php" method="POST" class="space-y-4">
            <input type="password" name="old_master" placeholder="Old Master Password" required class="w-full bg-[#0f0f0f] border border-gray-800 p-3 rounded-xl text-white outline-none">
	    <input type="hidden" name="csrf_token" value="<?= $csrf_token ?>">
            <input type="password" name="new_master" id="new_master" placeholder="New Master Password" required class="w-full bg-[#0f0f0f] border border-gray-800 p-3 rounded-xl text-white outline-none">
            <input type="password" name="confirm_master" id="confirm_master" placeholder="Confirm New Password" required class="w-full bg-[#0f0f0f] border border-gray-800 p-3 rounded-xl text-white outline-none">
            
            <div class="flex gap-3 mt-6">
                <button type="button" onclick="closeChangeMasterModal()" class="flex-1 py-3 bg-gray-800 text-gray-300 rounded-xl text-xs font-bold uppercase">Cancel</button>
                <button type="submit" class="flex-1 py-3 bg-blue-600 text-white rounded-xl text-xs font-bold uppercase">Confirm Change</button>
            </div>
        </form>
    </div>
</div>
